...
def add_plays():
    ...
    for channel in plays.keys():
        ...
        for start, (performer, title, length) in plays[channel].items():
            get_response(
                "add_performer", {"name": performer.encode("utf8")}, method=POST
            )
            get_response(
                "add_song",
                {"performer": performer.encode("utf8"), "title": title.encode("utf8")},
                method=POST,
            )
...
def get_song_plays(song):
    ...
    for channel in plays.keys():
        banned = [
            date
            for date in song_plays[channel].keys()
            if song_plays[channel][date] != song
        ]
        for b in banned:
...
def check_song_plays():
    ...
    for data in song3_plays:
        ...
        ] = (song3[0], song3[1], (end - start).seconds)
        ...
        res.append(
            {
                ...
                "title": song[1],
                "plays": pc[song],
                "previous_plays": previous_pc[song],
                "previous_rank": prev,
            }
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-H", action="store", dest="hostname", default="localhost", type=str
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="add_data",
        help=("Insert test data (only use the first time you " "run the script"),
        default=False,
    )
